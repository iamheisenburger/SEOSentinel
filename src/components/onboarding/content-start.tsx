...
export function ContentStart() {
  ...
  if (siteId) return <div className="mx-auto max-w-3xl space-y-4"><h1 className="text-xl font-semibold">Finish your content setup</h1>
    <p>Your saved profile is ready to review. Connect the exact destination, select page permissions, then confirm the service and schedule. Nothing is active yet.</p>
    <Link className="underline" href={`/sites/${siteId}?tab=settings`}>Connect GitHub or WordPress in website settings</Link>
  ...
  return <section className="mx-auto max-w-2xl space-y-4 rounded-xl border border-white/10 p-5"><h1 className="text-xl font-semibold">Start your content service</h1>
    <p>Tell Pentra what your business does and who it serves. You will review the publishing destination, permissions, funding and schedule before work begins. No outreach setup is required.</p>
