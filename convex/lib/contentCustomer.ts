...
import { readProviderBudgetAuthorization, providerBudgetMonth } from "./providerBudgetAuthorization";
...
export async function contentFunding(ctx: QueryCtx | MutationCtx, site: Doc<"sites">, budgetMicroUsd: number | undefined) {
  ...
  const complete = rows.length <= 2000;
  const current = rows.filter(r => r.createdAt >= window.startAt && r.releasedAt === undefined);
