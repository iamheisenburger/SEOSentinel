...
export default defineSchema({
  ...
  articles: defineTable({
    ...
    deterministicQualityRepairAttemptVersion: v.optional(v.number()),
  ...
  jobs: defineTable({
    contentWork: v.optional(v.object({
      ...
      providerCalls: v.array(v.object({ key: v.string(), ceilingMicroUsd: v.number(),
        ...
        logicalKey: v.optional(v.string()), requestHash: v.optional(v.string()),
        reservationId: v.optional(v.id("provider_spend_reservations")),
        state: v.union(v.literal("started"), v.literal("completed"), v.literal("rejected")), actualMicroUsd: v.optional(v.number()),
        result: v.optional(v.any()), rejectionStatus: v.optional(v.number()), rejectionCode: v.optional(v.string()),
        rejectionTrackingVersion: v.optional(v.literal(1)), rejectionRequestId: v.optional(v.string()), rejectionRecordedAt: v.optional(v.number()),
        rejectionStage: v.optional(v.union(v.literal("prepare"), v.literal("review"))),
        creditRecovery: v.optional(v.object({ confirmedAt: v.number(), fundingReference: v.string(), requestedAt: v.optional(v.number()),
          sourceRunId: v.optional(v.id("autopilot_runs")), sourceEvidenceHash: v.optional(v.string()) })) })),
      recoveryAttempts: v.optional(v.number()),
      semanticAuditRepair: v.optional(v.object({ version: v.literal(1), appliedAt: v.number(), reference: v.string(),
        callKey: v.string(), requestHash: v.string(), resultHash: v.string(), articleHash: v.string(),
        previousStatus: v.string(), previousStage: v.string(), previousFailure: v.optional(v.string()),
        previousError: v.optional(v.string()), previousNextAttemptAt: v.optional(v.number()),
        workerAttempts: v.number(), recoveryAttempts: v.number() })),
  ...
  provider_spend_reservations: defineTable({
    ...
    // Original ceiling remains immutable. Settlement records either exact
    // provider cost or the full spent-execution ceiling after permanently
    // retiring an unused single-execution-plan contingency. The reason tells
    // these apart; ambiguous/in-flight work retains the full reservation.
  ...
  outreach_contacts: defineTable({
    ...
    recipientConsentStatus: v.optional(v.union(
      v.literal("verified"),
      v.literal("revoked"),
    )),
    recipientConsentSource: v.optional(v.union(
      v.literal("web_form"),
    ...
    recipientConsentEvidenceHash: v.optional(v.string()),
    recipientConsentPurpose: v.optional(v.literal("commercial_email")),
    recipientConsentRecordedAt: v.optional(v.number()),
    recipientConsentExpiresAt: v.optional(v.number()),
    recipientConsentRevokedAt: v.optional(v.number()),
    recipientConsentPolicyVersion: v.optional(v.number()),
